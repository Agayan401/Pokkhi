...
function updateStatistics() {
    ...
    document.getElementById("lastUpdated").textContent = "13-Sept-2026";
}
...
// Add a temporary history entry while the bird details are open.
// This allows the mobile browser's back/swipe gesture to close
// the modal instead of leaving Pokkhi.
if (!history.state || !history.state.pokkhiModal) {
    history.pushState(
        { pokkhiModal: true },
        "",
...
function closeBirdModal(fromHistory = false) {

    if (!birdModal) return;

    birdModal.style.display = "none";

    // If the modal was closed normally using X or a swipe,
    // remove the temporary history entry.
    if (!fromHistory && history.state && history.state.pokkhiModal) {
        history.back();
    }
}
...
if (birdModal) {
    ...
    birdModal.addEventListener("touchmove", event => {
        ...
        /*
         * Detect horizontal movement early.
         *
         * This is important because waiting until touchend
         * allows some mobile browsers to begin their own
...
/* ==========================================
   MOBILE BACK / SWIPE-BACK HANDLING
   ========================================== */
...
window.addEventListener("popstate", event => {
    ...
    if (
        birdModal &&
        birdModal.style.display === "flex"
    ) {

        // Browser back/swipe gesture:
        // close the modal instead of leaving Pokkhi.
        closeBirdModal(true);
